...
class MetadataTimeChanger:
    ...
    def parse_time_adjustment(self, time_string: str) -> timedelta:
        ...
        if not time_string.startswith(('+', '-')):
            raise TimeParsingError(f"Time adjustment must start with + or -: {time_string}")
    ...
    def find_media_files(self) -> List[Path]:
        ...
        for current_file_path in self.source_path.rglob("*"):
            if self._is_supported_media_file(current_file_path, all_supported_extensions):
                discovered_media_files.append(current_file_path)
        
        return discovered_media_files
    
    def _is_supported_media_file(self, file_path: Path, supported_extensions: set) -> bool:
        """Check if a file is a supported media file."""
        return file_path.is_file() and file_path.suffix.lower() in supported_extensions
    
    def read_photo_metadata_timestamps(self, file_path: Path) -> Dict[str, Optional[datetime]]:
        """
    ...
    def _extract_exif_timestamps_piexif(self, exif_dict: dict) -> Dict[str, Optional[datetime]]:
        ...
        for field_id, field_name in exif_fields.items():
            try:
                ...
                elif field_id == piexif.ImageIFD.DateTime and '0th' in exif_dict and field_id in exif_dict['0th']:
    ...
    def _extract_exif_timestamps_pil(self, exif_data: dict) -> Dict[str, Optional[datetime]]:
    ...
    def read_video_metadata_timestamps(self, file_path: Path) -> Dict[str, Optional[datetime]]:
        """
        ...
        try:
            ...
            for track in media_info.tracks:
                if track.track_type == 'General':
                    ...
                    timestamp_fields = [
                        'recorded_date', 'tagged_date', 'encoded_date',
                    ...
                    for field_name in timestamp_fields:
                        try:
                            ...
                            if field_value:
                                timestamps[field_name] = self._parse_video_datetime_string(str(field_value))
                            else:
                                timestamps[field_name] = None
                        except Exception:
                            timestamps[field_name] = None
    ...
    def adjust_timestamps(self, timestamps: Dict[str, Optional[datetime]]) -> Dict[str, Optional[datetime]]:
        """
    ...
    def write_photo_metadata_timestamps(self, file_path: Path, timestamps: Dict[str, Optional[datetime]]) -> bool:
        """
        ...
        """
        if not piexif:
            self.errors.append(f"piexif not available for writing metadata to {file_path}")
            return False
    ...
    def write_video_metadata_timestamps(self, file_path: Path, timestamps: Dict[str, Optional[datetime]]) -> bool:
        """
        ...
        timestamp_priority = ['creation_time', 'recorded_date', 'date', 'encoded_date', 'tagged_date', 'mastered_date']
        ...
        if file_path.suffix.lower() == '.avi':
            return self.write_avi_metadata_safe_inplace_modify(file_path, primary_timestamp)
        
        # Use ffmpeg for other video formats
        return self.write_video_metadata_with_ffmpeg(file_path, primary_timestamp)
    
    def write_video_metadata_with_ffmpeg(self, file_path: Path, timestamp: datetime) -> bool:
        """
        ...
        if not ffmpeg or not subprocess:
            self.errors.append(f"ffmpeg not available for writing video metadata to {file_path}")
            return False
        
        # Check if ffmpeg is installed on the system
        if not shutil.which('ffmpeg'):
            self.errors.append(f"ffmpeg binary not found - cannot modify video metadata for {file_path}")
            return False
        ...
        try:
            ...
            cmd = [
                ...
                '-map_metadata', '0',  # Preserve all existing metadata
                '-metadata', f'creation_time={formatted_timestamp}',
                '-metadata', f'date={formatted_timestamp}',
            ...
            if temp_output.exists():
                ...
                self.errors.append(f"ffmpeg failed to create output file for {file_path}")
                return False
    ...
    def set_file_system_timestamps(self, file_path: Path, timestamps: Dict[str, datetime]) -> bool:
        """
        ...
        try:
            ...
            if 'modification_time' in timestamps and timestamps['modification_time'] is not None:
                target_timestamp = timestamps['modification_time']
            elif 'creation_time' in timestamps and timestamps['creation_time'] is not None:
            ...
            self.errors.append(f"Could not set file system timestamps for {file_path}: {e}")
            return False
    ...
    def process_single_file(self, file_path: Path) -> Dict[str, any]:
        ...
        try:
            ...
            if file_path.suffix.lower() in self.IMAGE_EXTENSIONS:
                original_metadata_timestamps = self.read_photo_metadata_timestamps(file_path)
            elif file_path.suffix.lower() in self.VIDEO_EXTENSIONS:
                original_metadata_timestamps = self.read_video_metadata_timestamps(file_path)
            else:
            ...
            # Adjust timestamps
            adjusted_metadata_timestamps = self.adjust_timestamps(original_metadata_timestamps)
            adjusted_filesystem_timestamps = self.adjust_timestamps(original_filesystem_timestamps)
            
            # Make creation time equal to modification time for consistency
            if 'modification_time' in adjusted_filesystem_timestamps and adjusted_filesystem_timestamps['modification_time'] is not None:
                adjusted_filesystem_timestamps['creation_time'] = adjusted_filesystem_timestamps['modification_time']
            ...
            should_write_metadata = (
                any(ts is not None for ts in adjusted_metadata_timestamps.values()) or
                (file_path.suffix.lower() == '.avi')
            )
            ...
            if should_write_metadata:
                if file_path.suffix.lower() in self.IMAGE_EXTENSIONS:
                    ...
                    if all(ts is None for ts in adjusted_metadata_timestamps.values()):
                        adjusted_metadata_timestamps['creation_time'] = adjusted_filesystem_timestamps['modification_time']
...
Supported time units:
  y = years, m = months, w = weeks, d = days, h = hours
    ...
    try:
        ...
        print(f"{'Would process' if parsed_arguments.dry_run else 'Processing'} files...")
        print()
        ...
        for file_path in media_files:
            ...
            if result['processed']:
                ...
                print(f"{'[DRY RUN] ' if parsed_arguments.dry_run else ''}Processing: {file_path}")
        ...
        print(f"Files with metadata {'would be ' if parsed_arguments.dry_run else ''}updated: {metadata_updated_count}")
        print(f"Files with filesystem timestamps {'would be ' if parsed_arguments.dry_run else ''}updated: {filesystem_updated_count}")
        
        # Show errors in red if any
        if changer.errors:
            print()
            print(f"\033[91mERRORS ENCOUNTERED ({len(changer.errors)}):\033[0m")  # Red text
