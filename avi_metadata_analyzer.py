...
def analyze_avi_metadata(file_path):
        ...
        if result.returncode == 0:
            ...
            if 'format' in data and 'tags' in data['format']:
                ...
                for key, value in data['format']['tags'].items():
                    if any(date_word in key.lower() for date_word in ['date', 'time', 'create', 'record', 'origin']):
                        print(f"      🏷️  {key}: {value}")
            ...
            if 'streams' in data:
                for i, stream in enumerate(data['streams']):
                    if 'tags' in stream:
                        print(f"   🎞️  STREAM {i} TAGS ({stream.get('codec_type', 'unknown')}):")
    ...
    try:
        cmd = ['ffprobe', '-v', 'quiet', '-show_entries', 'format_tags', '-of', 'csv=p=0', str(file_path)]
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
        if result.returncode == 0:
    ...
    try:
        ...
        for track in info.tracks:
            ...
            for key, value in track_data.items():
                if value and any(date_word in key.lower() for date_word in ['date', 'time', 'create', 'record', 'origin', 'master']):
        ...
        with open(file_path, 'rb') as f:
            ...
            for sig in signatures:
                ...
                if pos != -1:
                    ...
                    readable = ''.join(chr(b) if 32 <= b <= 126 else f'\\x{b:02x}' for b in context)
                    found_metadata.append(f"   🔍 Found {sig.decode('ascii', errors='ignore')} at offset {pos}: {readable}")
    ...
    print("📋 Look for date fields that might correspond to Windows File Explorer 'Media created'")
    print("=" * 80)
